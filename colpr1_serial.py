...
def splan3(data: ComclhmgData):
    ...
    while True:
        ...
        alfa3 = (data.g[0] - data.g[1]) / (data.x[0] - data.x[1]) -\
...
def homog(data: ComclhmgData):
    ...
    for ig in range(data.imax):
        ...
        for inn in range(data.mclnuc):
            idsp = data.idcl[inn]
            data.sig0g[inn, ig] = data.sig0[inn]
            data.sigtg[inn, ig] = data.sigt[inn]
            
            nreac = 0
            fval = inter1(data, ig, nreac, idsp, data.sig0g[inn, ig], data.temper)
            data.sigf[inn, ig] = fval * data.sig1d[idsp, 1 - 1, ig]

            nreac = 1
            fval = inter1(data, ig, nreac, idsp, data.sig0g[inn, ig], data.temper)
            data.sigc[inn, ig] = fval * data.sig1d[idsp, 3 - 1, ig]

            nreac = 2
            fval = inter1(data, ig, nreac, idsp, data.sig0g[inn, ig], data.temper)
            data.sigel[inn, ig] = fval * data.sig1d[idsp, 5 - 1, ig]

            nreac = 4
            fval = inter1(data, ig, nreac, idsp, data.sig0g[inn, ig], data.temper)
            data.siger[inn, ig] = fval * data.sig1d[idsp, 7 - 1, ig]

            nreac = 5
            fval = inter1(data, ig, nreac, idsp, data.sig0g[inn, ig], data.temper)
            data.sigin[inn, ig] = fval * data.sig1d[idsp, 4 - 1, ig]

            data.sig2[inn, ig] = data.sig1d[idsp, 8 - 1, ig]
            data.xnu[inn, ig] = data.sig1d[idsp, 2 - 1, ig]
            data.xmyu[inn, ig] = data.sig1d[idsp, 6 - 1, ig]

            data.siga[inn, ig] = data.sigc[inn, ig] + data.sigf[inn, ig]
...
def incoef(data: ComclhmgData):
    with open('D:\Materi Sains Komputasi\Tesis\gausquad.dat', 'r') as file:  # Open the input file
        file.readline()
        file.readline()
...
def calcp0(data: ComclhmgData, ii, iord, ig):
    ...
    if ii > 0:
        ...
        for iteta in range(iord):
            rho = rrm / 2.0 * (data.absg[iord-2, iteta] + 1.0)
            xlamda = xlmdk(data, ii, ig, rho)
            xlamd1 = xlamda 
            xinteg += 2.0 * xlamda * data.weigg[iord-2, iteta]
            xinteg += 2.0 * data.weigg[iord-2, iteta] * xki3(xlamda, nbkf)
            xlamda = 0.0
            xinteg -= data.weigg[iord-2, iteta] * np.pi / 2.0 
            xlamd3 = xlmd3(data, ii, ig, rho)
            xinteg += data.weigg[iord-2, iteta] * xki3(xlamd3, nbkf)
            xlamda = xlamd3 + xlamd1
            xinteg -= 2.0 * data.weigg[iord-2, iteta] * xki3(xlamda, nbkf)
            xlamda = xlamd3 + 2.0 * xlamd1
            xinteg += data.weigg[iord-2, iteta] * xki3(xlamda, nbkf)
    ...
    for iteta in range(iord):
        rho = delr * (data.absg[iord-2, iteta] + 1.0) + rim
        xlamd1 = data.sigtrg[ii, ig] * np.sqrt(data.rrg[ii] ** 2 - rho ** 2)
        xlamda = xlamd1
        xsupl += 4.0 * xlamda * data.weigg[iord-2, iteta]
        xlamda = 2.0 * xlamd1
        xsupl += 2.0 * data.weigg[iord-2, iteta] * xki3(xlamda, nbkf)
        xlamda = 0.0
        xsupl -= data.weigg[iord-2, iteta] * np.pi / 2.0
...
def calcp(data: ComclhmgData, ii, jj, iord, ig):
    ...
    xinteg = 0.0
    for iteta in range(iord):
        rho = data.rrg[ii] / 2.0 * (data.absg[iord-2, iteta] + 1.0)
    ...
    for ig in range(data.imax):
        for ii in range(data.maxcrg):
            for jj in range(data.maxcrg):
                ...
                data.pij[ii, jj, ig] = colprb
            escprb = escpr(data, ii, data.nord, ig)
            data.pesc[ii, ig] = escprb
...
start = time.time()
read1(r'D:\Materi Sains Komputasi\Tesis\contohinput.inp', data)
