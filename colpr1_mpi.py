...
def homog(data: ComclhmgData):
    ...
    def process_ig2(data, ig, iig):
        ...
        for inn in range(data.mclnuc):
            idsp = data.idcl[inn]
            local_sig0g[inn, iig] = data.sig0[inn]
            local_sigtg[inn, iig] = data.sigt[inn]
            
            nreac = 0
            fval = inter1(data, ig, nreac, idsp, local_sig0g[inn, iig], data.temper)
            local_sigf[inn, iig] = fval * data.sig1d[idsp, 1 - 1, ig]

            nreac = 1
            fval = inter1(data, ig, nreac, idsp, local_sig0g[inn, iig], data.temper)
            local_sigc[inn, iig] = fval * data.sig1d[idsp, 3 - 1, ig]

            nreac = 2
            fval = inter1(data, ig, nreac, idsp, local_sig0g[inn, iig], data.temper)
            local_sigel[inn, iig] = fval * data.sig1d[idsp, 5 - 1, ig]

            nreac = 4
            fval = inter1(data, ig, nreac, idsp, local_sig0g[inn, iig], data.temper)
            local_siger[inn, iig] = fval * data.sig1d[idsp, 7 - 1, ig]

            nreac = 5
            fval = inter1(data, ig, nreac, idsp, local_sig0g[inn, iig], data.temper)
            local_sigin[inn, iig] = fval * data.sig1d[idsp, 4 - 1, ig]

            local_sig2[inn, iig] = data.sig1d[idsp, 8 - 1, ig]
            local_xnu[inn, iig] = data.sig1d[idsp, 2 - 1, ig]
            local_xmyu[inn, iig] = data.sig1d[idsp, 6 - 1, ig]

            local_siga[inn, iig] = local_sigc[inn, iig] + local_sigf[inn, iig]

           
    energy_groups = list(range(data.imax))
    chunks = np.array_split(energy_groups, size)
    my_chunk = chunks[rank]

    a, b = data.sig0g.shape
    local_sig0g = np.zeros((a, len(my_chunk)))
    local_sigtg = np.zeros((a, len(my_chunk)))
    local_sigf = np.zeros((a, len(my_chunk)))
    local_sigc = np.zeros((a, len(my_chunk)))
    local_sigel = np.zeros((a, len(my_chunk)))
    local_siger = np.zeros((a, len(my_chunk)))
    local_sigin = np.zeros((a, len(my_chunk)))
    local_sig2 = np.zeros((a, len(my_chunk)))
    local_xnu = np.zeros((a, len(my_chunk)))
    local_xmyu = np.zeros((a, len(my_chunk)))
    local_siga = np.zeros((a, len(my_chunk)))

    iig = 0
    for ig in my_chunk:
        process_ig2(data, ig, iig)
        iig += 1
    
    sig0g_gather = comm.gather(local_sig0g, root=0)
    sigtg_gather = comm.gather(local_sigtg, root=0)
    sigf_gather = comm.gather(local_sigf, root=0)
    sigc_gather = comm.gather(local_sigc, root=0)
    sigel_gather = comm.gather(local_sigel, root=0)
    siger_gather = comm.gather(local_siger, root=0)
    sigin_gather = comm.gather(local_sigin, root=0)
    sig2_gather = comm.gather(local_sig2, root=0)
    xnu_gather = comm.gather(local_xnu, root=0)
    xmyu_gather = comm.gather(local_xmyu, root=0)
    siga_gather = comm.gather(local_siga, root=0)

    comm.Barrier()

    if rank == 0:
        data.sig0g = np.hstack(sig0g_gather)
        data.sigtg = np.hstack(sigtg_gather)
        data.sigf = np.hstack(sigf_gather)
        data.sigc = np.hstack(sigc_gather)
        data.sigel = np.hstack(sigel_gather)
        data.siger = np.hstack(siger_gather)
        data.sigin = np.hstack(sigin_gather)
        data.sig2 = np.hstack(sig2_gather)
        data.xnu = np.hstack(xnu_gather)
        data.xmyu = np.hstack(xmyu_gather)
        data.siga = np.hstack(siga_gather)
...
def incoef(data: ComclhmgData):
    with open('D:\Tesis Ghulam Abrar\gausquad.dat', 'r') as file:
        file.readline()
        file.readline()
...
def colpr1(data: ComclhmgData):
    ...
    def process_ig(data, ig, iig):
        for ii in range(data.maxcrg):
            for jj in range(data.maxcrg):
                ii1 = ii
                jj1 = jj
                colprb = calcp(data, ii1, jj1, data.nord, ig)
                # print(f'reg: {ii}, {jj}, {ig}, cp: {colprb}')
                local_pij[ii, jj, iig] = colprb
            escprb = escpr(data, ii, data.nord, ig)
            local_pesc[ii, iig] = escprb
    
    # Divide the energy groups among the available processes
    energy_groups = list(range(data.imax))
    chunks = np.array_split(energy_groups, size)
    my_chunk = chunks[rank]
    ...
    d, e = data.pesc.shape
    local_pij = np.zeros((a, b, len(my_chunk)))
    local_pesc = np.zeros((d, len(my_chunk)))

    iig = 0
    for ig in my_chunk:
        process_ig(data, ig, iig)
        iig += 1
    
    pij_gather = comm.gather(local_pij, root=0)
    pesc_gather = comm.gather(local_pesc, root=0)

    comm.Barrier()

    if rank == 0:
        data.pij = np.dstack(pij_gather)
        data.pesc = np.hstack(pesc_gather)
...
start = time.time()
read1(r'D:\Tesis Ghulam Abrar\papertes.inp', data)
